...
trap `race-report` §0 already warns about for a different tool, one
level up.
...
default, and updates `race-report`'s §1–§2 with the same caution:
...
- **#206** — `fetch_debug_artifacts.py logs` (and the `race-report`
