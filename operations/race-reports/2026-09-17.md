...
`20260917_*` directory. Per `race-report` §2 the empty `logs` result was
...
| time | event |
|---|---|
| 06:29:04 | "for @rongarner book 9/23 at 3:08p" received, target resolved to @rongarner's stored credential |
...
account is scoped to storage + logging (`operations/debug-artifact-access.md`), so
...
- **There is no Friday-specific behaviour anywhere in this codebase.** Every
  ...
  §7f of the `race-report` skill discusses Fridays because that is when
