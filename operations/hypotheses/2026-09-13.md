...
|---|---|---|---|---|---|
| Group X | 2026-09-20 | 12:00 PM | 4 | 18:01:50 CT | 18:02:01 CT |
| Group Y | 2026-09-20 | 12:08 PM | 4 | 18:31:19 CT | 18:31:36.020 CT |
| Group X | 2026-09-20 | 12:00 PM | 4 | 18:30:05 CT | 18:31:36.872 CT |

All three from Telegram chat `-5588592546` (Cloud Run logs, `resource.labels.service_name="teetime"`,
2026-09-12 17:00-19:00 UTC window). The third row is Group X re-asking for the exact slot
...
`SCHEDULED` rows, not two, and one requester (Group X) is unknowingly double-booked against
themselves.
...
requested_time)` before writing a new one. Group X asked twice, 29 minutes apart, and
...
- **Group X**: two requests, both 12:00 PM.
- **Group Y**: one request, 12:08 PM.
...
Group X's requests target the identical 12:00 PM slot. Whichever is processed first will
...
**Prediction: one of Group X's two bookings SUCCEEDS, the other FAILS with a
...
**which includes 12:08 PM, Group Y's exact target.** If Group X runs first (2d) and its
losing request falls back onto 12:08 before Group Y's session has even logged in, Group Y
...
rows close to insertion order — and Group X's first booking was created at 18:02:01 CT, ~30
minutes before Group Y's only booking (18:31:36.033) or Group X's second
(18:31:36.872).

**Prediction: Group X executes first and gets the pre-window
login/nav/scroll head start described in 2d; Group Y goes second, cold.**
...
**Prediction: all three outcome messages (two for Group X, one for Group Y) land in the
...
- [ ] Did Group X's second request fail with a `Restriction:` popup
  (`phase=player_count/tbd_guests/book_now` per the race-report skill's §6), or something else —
...
- [ ] Did the losing request's fallback walk land on 12:08 PM, and if so, before or after
  Group Y's own Reserve was sent for it?
- [ ] What `sentMsPastWindow` did Group Y's first Reserve actually carry, versus Group X's?
- [ ] Did Group Y get 12:08 PM, a fallback, or nothing?
...
Related: `operations/race-reports/2026-09-11.md`, `.claude/skills/race-report/SKILL.md`.
