...
class WaldenGolfProvider(ReservationProvider):
    ...
    def _select_date_sync(self, driver: webdriver.Chrome, target_date: date) -> bool:
        """Point the tee sheet at ``target_date``.

        The routine itself lives in ``walden_date_selection`` because the
        observer needs the same one and cannot import this module - it is held
        to a read-only guarantee by tests that forbid the import path (issue
        #199). Behaviour here is unchanged bar one addition: a calendar that
        fails now falls back to paging the date strip, where it previously
        abandoned the booking.
        """
        return walden_date_selection.select_date(
            driver,
            target_date,
            wait_strategy=self.wait_strategy,
            log_prefix="BOOKING_DEBUG",
        )
